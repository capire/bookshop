...
const { decorateClient, isDynatraceEnabled } = require('./dynatrace')
...
const _connectHdb = (creds, tenant) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const decoratedClient = isDynatraceEnabled() ? decorateClient(client, tenant) : client

    decoratedClient.connect(err => {
      _registerReconnect(decoratedClient, creds)
      if (err) reject(_ensureError(err))
      else {
        if (creds.schema) {
          decoratedClient.exec(`SET SCHEMA ${creds.schema}`, err => {
            if (err) reject(Object.assign(new Error('Could not set schema'), err))
            else resolve(decoratedClient)
          })
        } else resolve(decoratedClient)
      }
    })
...
function factory4 (creds, tenant) {
  return {
    create: function () {
      if (hana.name === 'hdb') {
        return _connectHdb(creds, tenant)
      }
    ...
    pools.set(
      ...
      new Promise((resolve, reject) => {
        credentials4(tenant, credentials)
          .then(creds => {
            resolve(pool.createPool(factory4(_getMassagedCreds(creds), tenant), _getPoolConfig()))
          })
          .catch(e => {
